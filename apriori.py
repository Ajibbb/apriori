...
# load data 
df = pd.read_excel('bread_basket1.xlsx')
...
st.title("PT Smith Indonesia Jaya")

def get_data(periode_day = '', weekday_weekend = '', month = '', day = ''):
    data = df.copy()
    filtered = data.loc[
...
def user_input_features():
    item = st.selectbox("Item",{'Hair Clay & Hairwax Bundling', 'Ocean Dust Bundling & Ocean Dust & Refresh Wipes', 'Dandruff Buster Shampoo', 'Ocean Dust & Dapper Statter', 'Ocean Dust & Hair Clay', 'Pomade Oil Based Fine Shinee', 'Ocean Dust & Clayton', 'Ocean Dust & Pomade Dapper Stater', 'Shampoo & Face Cleanser & Body Cleanser', 'Refresh Wipes', 'Pomade Oil Based Premium Medium', 'BubbleWrap Tambahan', 'Tote Bag', 'Tshirt', 'Deep Face Cleanser Facial Wash', 'GingerHair Clay & Hairwax Bundling  syrup', 'Hair Clay & Hairwax Bundling Pudding', 'Hair Powder Ocean Dust Team NXL', 'Hair Styling &  Volumizing Powder Ocean Dust', 'Hair Styling &  Volumizing Powder Ocean Dust & Volumizing Powder Ocean Dust', 'Hairwax Black Jack', 'Pomade Water Based Dapper Stater', 'Oil Based Pomade Choco Loco', 'Pomade Water Based Dashing Slick', 'Smith Tshirt Collection SS19',} )
...
def return_item_df(item_antecedents):
    ...
    return list(data.loc[data["antecedents"] == item_antecedents].iloc[5,:])
